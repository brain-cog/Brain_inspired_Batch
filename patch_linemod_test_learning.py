...
dp = get_dataset_params(dataset)               #get the paramters of dataset ->dp
detector = linemodLevelup_pybind.Detector(16, [4, 8], 16)  # min features; pyramid strides(bufu); num clusters=16

obj_ids = []  # for each obj
obj_ids_curr = range(1, dp['obj_count'] + 1)           #num of obj etc: 1 2 ....obj_count
if obj_ids:
    obj_ids_curr = set(obj_ids_curr).intersection(obj_ids)      #find the same  element in set(jihe) obj_ids and obj_ids_curr ,then return a new set
...
dep_max = dp['test_obj_depth_range'][1]  # max depth of scene  200mm
dep_anchor_step = 1.2  # depth scale(bili) 120mm or 100mm(depth anchor)
...
current_dep = dep_min
while current_dep < dep_max:                #BFS points depth<=200mm
...
top_level_path = os.path.dirname(os.path.abspath(__file__))                   #dirname get the file's parent items ;abspath the current items
template_saved_to = join(dp['base_path'], 'linemod_render_up', '%s.yaml')       #creat the path file
...
misc.ensure_dir(os.path.dirname(template_saved_to))     #make sure the path exists
misc.ensure_dir(os.path.dirname(tempInfo_saved_to))
misc.ensure_dir(result_base_path)

if mode == 'render_train':           #train the models in different radius depth scales
    start_time = time.time()

    im_size = dp['cam']['im_size']      #image size
    ...
    color_buf = np.zeros((shape[0], shape[1], 4), np.float32).view(renderer.gloo.TextureFloat2D)    #numpy->calc array matrix 
    depth_buf = np.zeros((shape[0], shape[1]), np.float32).view(renderer.gloo.DepthTexture)           #texture information from the train
    fbo = renderer.gloo.FrameBuffer(color=color_buf, depth=depth_buf)
    fbo.activate()        #start fbo to store  (color depth)

    for obj_id in obj_ids_curr:
        azimuth_range = dp['test_obj_azimuth_range']            #fangweijiao
        elev_range = dp['test_obj_elev_range']                  #yangjiao
        min_n_views = 200
        clip_near = 10  # [mm]          the range of points?
        clip_far = 10000  # [mm]
        ambient_weight = 0.8  # Weight of ambient light [0, 1]
        shading = 'phong'  # 'flat', 'phong'(OpenGL) flat draw clors by polygon ; phong is by pixel

        # Load model
        model_path = dp['model_mpath'].format(obj_id)
        model = inout.load_ply(model_path)          #loal the models.ply
        ...
        surf_color = None
        mode = 'rgb+depth'
        K = dp['cam']['K']     # param K: Camera matrix.

        assert ({'pts', 'faces'}.issubset(set(model.keys())))   #issubest->whether all the elements in  set(jihe)? if not assert->break down
        # Set texture / color of vertices(dingdian)
        if texture is not None:
            if texture.max() > 1.0:
                texture = texture.astype(np.float32) / 255.0            #astype:change the type of array
            texture = np.flipud(texture)                                #fanzhuan matrix(juzhen)
            texture_uv = model['texture_uv']
            colors = np.zeros((model['pts'].shape[0], 3), np.float32)   #return the given (shape size) array  zero->all 0 in array
        else:
            ...
            if not surf_color:
                if 'colors' in model.keys():
                    ...
                    colors = np.ones((model['pts'].shape[0], 3), np.float32) * 0.5              #ones->all 1 in array
            else:
                colors = np.tile(list(surf_color) + [1.0], [model['pts'].shape[0], 1])             #title->repeat the array

        # Set the vertex(dingdian) data
        if mode == 'depth':
        ...
        # Projection(youying) matrix
        mat_proj = renderer._compute_calib_proj(K, 0, 0, im_size[0], im_size[1], clip_near, clip_far) #(K, x0, y0, Image width w, h, nc, fc, window_coords='y_down') ;param K->Camera matrix.

        # Model matrix
        mat_model = np.eye(4, dtype=np.float32)  # From object space to world space  eye:creat duijiao matrix
        ...
        # OpenGL setup
        renderer.gl.glEnable(renderer.gl.GL_DEPTH_TEST)  #open OpenGL
        renderer.gl.glViewport(0, 0, shape[1], shape[0])   #(zuoxiaojiao x,y,window width,height)
        renderer.gl.glDisable(renderer.gl.GL_CULL_FACE)     #close
        ######################################################
        ...
        if fast_train:
            ...
            for view_id, view in enumerate(views):    #enumerate find all the data and set a suoyin sequence and list the data data_xiabiao

                if view_id % 10 == 0:
                    print('obj,radius,view: ' + str(obj_id) +
                          ',' + str(dep_anchors[0]) + ',' + str(view_id) + ', view_id: ', view_id)    #radius means the depth_anchors like 100mm pick the point
                ...
                mat_view = mat_view.T  # OpenGL expects column-wise(zhuzhuang) matrix format
                ...
                rgb = cv2.resize(rgb, dp['cam']['im_size'], interpolation=cv2.INTER_AREA)   #suofang

                # have read rgb, depth, pose, obj_bb, obj_id here

                rows = np.any(depth, axis=1)        #any->test whether array have any true    all-> all array have true 
                cols = np.any(depth, axis=0)
                ymin, ymax = np.where(rows)[0][[0, -1]]   #np.where(condition x y)
            ...
            for radius in dep_anchors:
                ...
                # Render the object model from all the views
                for view_id, view in enumerate(views):    #find all the data and list 
                    window.clear()
                    ...
                    ################################################################
                    mat_view = np.eye(4, dtype=np.float32)  # From world space to eye space
                    mat_view[:3, :3] = view['R']            #R(rodrigues bianhuan) xuanzhuan bianhuan?
                    mat_view[:3, 3] = view['t'].squeeze()
                    yz_flip = np.eye(4, dtype=np.float32)
                    yz_flip[1, 1], yz_flip[2, 2] = -1, -1
                    mat_view = yz_flip.dot(mat_view)  # OpenCV to OpenGL camera system
                    ...
                    renderer.gl.glClearColor(0.0, 0.0, 0.0, 0.0)            #clear(R G B AFA(alpha toumingdu))  two windows->rgb and mask
                    renderer.gl.glClear(renderer.gl.GL_COLOR_BUFFER_BIT | renderer.gl.GL_DEPTH_BUFFER_BIT)

                    program_dep['u_mv'] = renderer._compute_model_view(mat_model, mat_view)   #matrix (model view) multiplicative
                    ...
                    # Retrieve the contents of the FBO texture   #fbo store  (color depth)
                    ...
                    # # Render RGB image  #shua xin color
                    ##################################################################
                    renderer.gl.glClearColor(bg_color[0], bg_color[1], bg_color[2], bg_color[3])   #just set value :RGB alpha  two windows->rgb and mask
                    renderer.gl.glClear(renderer.gl.GL_COLOR_BUFFER_BIT | renderer.gl.GL_DEPTH_BUFFER_BIT)   #clear as the value
                    ...
                    rgb = cv2.resize(rgb, dp['cam']['im_size'], interpolation=cv2.INTER_AREA)   # ---->rgb

                    K = dp['cam']['K']    # K = dp['cam']['K']     # param K: Camera matrix. # ---->depth
                    R = view['R']      #ROTATE matrix
                    t = view['t']       #pose matrix
                    # have read rgb, depth, pose, obj_bb, obj_id here
    ...
    fbo.deactivate()        #start to store rgbd
    window.close()
...
if mode == 'test':
    poseRefine = linemodLevelup_pybind.poseRefine()   #refine the poserefine to store the pose information

    im_size = dp['test_im_size']
    shape = (im_size[1], im_size[0])    #Image height width
    print('test img size: {}'.format(shape))

    # Frame buffer object, bind here to avoid memory leak, maybe?
    window = renderer.app.Window(visible=False)
    color_buf = np.zeros((shape[0], shape[1], 4), np.float32).view(renderer.gloo.TextureFloat2D)     #fix here?!!! to use camera RGBD
    depth_buf = np.zeros((shape[0], shape[1]), np.float32).view(renderer.gloo.DepthTexture)
    fbo = renderer.gloo.FrameBuffer(color=color_buf, depth=depth_buf)
    fbo.activate()

    use_image_subset = True   #subset ziji
    if use_image_subset:
        im_ids_sets = inout.load_yaml(dp['test_set_fpath'])    #load the train infromation /home/casia/patch_linemod/public/datasets/doumanoglou/linemod_renderup
    else:
        im_ids_sets = None

    for scene_id in scene_ids_curr:     #scene in test
        obj_id_in_scene_array = list()
        obj_id_in_scene_array.append(scene_id)

        if dataset =='doumanoglou' and scene_id == 3:     #three tests
        ...
        for obj_id_in_scene in obj_id_in_scene_array:
            ...
            model = inout.load_ply(dp['model_mpath'].format(obj_id_in_scene))       #get the train information
            ...
            active_ratio = 0.6      #the part allowed to be covered
            for im_id in im_ids_curr:
                ...
                # Load the images
                rgb = inout.load_im(dp['test_rgb_mpath'].format(scene_id, im_id))                  #fix here to change the picture and use camera RGBD
                depth = inout.load_depth(dp['test_depth_mpath'].format(scene_id, im_id))
                depth *= dp['cam']['depth_scale']
                depth = depth.astype(np.uint16)  # [mm]
                im_size = (depth.shape[1], depth.shape[0])    #width height
                ...
                matches = detector.match([rgb, depth], 70, active_ratio,                #70 threshold ->the least score in one patch()
                                         match_ids, dep_anchors, dep_range, masks=[])     #match the images  

                depth_edge = poseRefine.get_depth_edge(depth)  # already dilute(xishi) & distant transform
                ...
                print('candidates size before refine & nms: {}\n'.format(len(matches)))     #houxuan feture point

                dets = []           #dets server(file manage) and dets(read the hed and freelist of files)
                Rs = []
                Ts = []
                icp_scores = []
                ...
                top100_local_refine = 100  # avoid too many for simple obj,                fetures numbers that allowed 
                ...
                for i in range(top100_local_refine):
                    ...
                    window.clear()
                    renderer.gl.glClearColor(0.0, 0.0, 0.0, 0.0)    #rgb_top window?
                    renderer.gl.glClear(renderer.gl.GL_COLOR_BUFFER_BIT | renderer.gl.GL_DEPTH_BUFFER_BIT)
